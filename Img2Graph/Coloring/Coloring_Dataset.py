from Any2Graph.base_dataset_class import Dataset
...
class ColoringDataset(Dataset):
    ...
    def __init__(self,root_path='Img2Graph/Coloring/data/',subset='small',split='test',augment_data=False,dataset_size=0):
        
        super().__init__()
        
        root_path = os.path.join(root_path,'coloring_'+subset,split)
    ...
    def __getitem__(self, idx):
        ''' Must return: image in torch format (C,H,W), graph in dic format {'F':,'A':,'h':}, idx   '''
        ...
        A = to_dense_adj(graph.edge_index,max_num_nodes=graph.num_nodes,edge_attr=graph.edge_weight).squeeze()
        F = graph.F.to(torch.float32)
        F_tilde = A.float()@F
        h = graph.h.to(torch.float32)
        y = {'A':A,'F':F,'F_tilde':F_tilde,'h':h,'G':None}
    ...
    def plot_trgt(self,index_trgt,ax_trgt,frame=False):
        ...
        edges_weights = [2*graph_trgt_nx[u][v]["edge_weight"] for u,v in graph_trgt_nx.edges]
    ...
    def plot_pred_trgt(self,F,A,h,index_trgt,ax_trgt,ax_pred,frame=False):
