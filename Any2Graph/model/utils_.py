...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        encoder_layer = TransformerEncoderLayer(model_dim, nhead, 4*model_dim,
                                                dropout, activation, normalize_before)
        encoder_norm = nn.LayerNorm(model_dim) if normalize_before else None
        self.encoder = TransformerEncoder(encoder_layer, num_layer, encoder_norm)

        decoder_layer = TransformerDecoderLayer(model_dim, nhead, 4*model_dim,
                                                dropout, activation, normalize_before)
    ...
    def forward(self, src, mask, pos_embed):
        ...
        B, d, N = src.shape        
        src = src.permute(1, 0, 2) # Number of Features first
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            x = F.dropout(F.relu(layer(x)),p=self.dropout,training=self.training) if i < self.num_layers - 1 else layer(x)
        return x